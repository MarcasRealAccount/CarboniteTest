...
namespace ResourceManager
{
		...
		if (iterator != m_Regions.end())
		{
			if (iterator->m_End == m_CurrentIndex - 1)
			{
				...
				for (auto it = iterator + 1; it != m_Regions.end(); ++it)
					++it->m_Offset;
				m_Resource.emplace(m_Resources.begin() + iterator->m_Offset + (iterator->m_End - iterator->m_Start), std::forward<Args>(args)...);
			...
			if (iterator->m_Start == m_CurrentIndex + 1)
			{
				...
				for (auto it = iterator + 1; it != m_Regions.end(); ++it)
					++it->m_Offset;
				m_Resource.emplace(m_Resources.begin() + iterator->m_Offset, std::forward < Args(args)...);
				nextOptimalIndex();
	...
	ResourcePool<T, IndexType>::Ref ResourcePool<T, IndexType>::emplaceBackPersistent(Args&&... args) requires std::is_constructible_v<T, Args...>
	{
		...
		auto resource = getResoucePtr(ref.index());
	...
	template <class T, class IndexType>
	const ResourcePool<T, IndexType>::Resource* ResourcePool<T, IndexType>::getResourcePtr(IndexT index) const
	{
		auto region = getRegion(index);
		return region == m_Regions.end() ? nullptr : &m_Resources[region->m_Offset + (index - region->m_Start)];
	}

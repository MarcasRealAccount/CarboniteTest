...
namespace Input
{
	...
	{
	public:
		constexpr Binding(EInputLocation location, std::uint32_t id, std::uint32_t index, float sensitivity = 1.0f)
		    : m_Location(location), m_ID(id), m_Index(index), m_Sensitivity(sensitivity) {}
	...
	public:
		ButtonBinding(const std::string& name, Binding button)
		    : m_Name(name), m_Button(button), m_State(0) {}
		ButtonBinding(std::string&& name, Binding button)
		    : m_Name(std::move(name)), m_Button(button), m_State(0) {}
	...
	public:
		AxisBinding(const std::string& name, Binding axis, EAxisMode mode)
		    : m_Name(name), m_Axis(axis), m_Mode(mode), m_State(0.0f), m_PState(0.0f) {}
		AxisBinding(std::string&& name, Binding axis, EAxisMode mode)
		    : m_Name(std::move(name)), m_Axis(axis), m_Mode(mode), m_State(0.0f), m_PState(0.0f) {}
	...
	public:
		Axis2DBinding(const std::string& name, Binding xAxis, Binding yAxis, EAxisMode mode)
		    : m_Name(name), m_XAxis(xAxis), m_YAxis(yAxis), m_Mode(mode), m_State({ 0.0f, 0.0f }), m_PState({ 0.0f, 0.0f }) {}
		Axis2DBinding(std::string&& name, Binding xAxis, Binding yAxis, EAxisMode mode)
		    : m_Name(std::move(name)), m_XAxis(xAxis), m_YAxis(yAxis), m_Mode(mode), m_State({ 0.0f, 0.0f }), m_PState({ 0.0f, 0.0f }) {}
	...
	public:
		Axis3DBinding(const std::string& name, Binding xAxis, Binding yAxis, Binding zAxis, EAxisMode mode)
		    : m_Name(name), m_XAxis(xAxis), m_YAxis(yAxis), m_ZAxis(zAxis), m_Mode(mode), m_State({ 0.0f, 0.0f, 0.0f }), m_PState({ 0.0f, 0.0f, 0.0f }) {}
		Axis3DBinding(std::string&& name, Binding xAxis, Binding yAxis, Binding zAxis, EAxisMode mode)
		    : m_Name(std::move(name)), m_XAxis(xAxis), m_YAxis(yAxis), m_ZAxis(zAxis), m_Mode(mode), m_State({ 0.0f, 0.0f, 0.0f }), m_PState({ 0.0f, 0.0f, 0.0f }) {}
	...
	public:
		ButtonAxisBinding(const std::string& name, Binding posButton, Binding negButton)
		    : m_Name(name), m_PosButton(posButton), m_NegButton(negButton), m_State(0.0f) {}
		ButtonAxisBinding(std::string&& name, Binding posButton, Binding negButton)
		    : m_Name(std::move(name)), m_PosButton(posButton), m_NegButton(negButton), m_State(0.0f) {}
	...
	public:
		ButtonAxis2DBinding(const std::string& name, Binding posXButton, Binding negXButton, Binding posYButton, Binding negYButton)
		    : m_Name(name), m_PosXButton(posXButton), m_NegXButton(negXButton), m_PosYButton(posYButton), m_NegYButton(negYButton), m_State({ 0.0f, 0.0f }) {}
		ButtonAxis2DBinding(std::string&& name, Binding posXButton, Binding negXButton, Binding posYButton, Binding negYButton)
		    : m_Name(std::move(name)), m_PosXButton(posXButton), m_NegXButton(negXButton), m_PosYButton(posYButton), m_NegYButton(negYButton), m_State({ 0.0f, 0.0f }) {}
	...
	public:
		ButtonAxis3DBinding(const std::string& name, Binding posXButton, Binding negXButton, Binding posYButton, Binding negYButton, Binding posZButton, Binding negZButton)
		    : m_Name(name), m_PosXButton(posXButton), m_NegXButton(negXButton), m_PosYButton(posYButton), m_NegYButton(negYButton), m_PosZButton(posZButton), m_NegZButton(negZButton), m_State({ 0.0f, 0.0f, 0.0f }) {}
		ButtonAxis3DBinding(std::string&& name, Binding posXButton, Binding negXButton, Binding posYButton, Binding negYButton, Binding posZButton, Binding negZButton)
		    : m_Name(std::move(name)), m_PosXButton(posXButton), m_NegXButton(negXButton), m_PosYButton(posYButton), m_NegYButton(negYButton), m_PosZButton(posZButton), m_NegZButton(negZButton), m_State({ 0.0f, 0.0f, 0.0f }) {}
...
namespace Input
{
	namespace Buttons
	{
		...
		constexpr Binding GamepadA(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadA, sensitivity }; }
		constexpr Binding GamepadB(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadB, sensitivity }; }
		constexpr Binding GamepadX(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadX, sensitivity }; }
		constexpr Binding GamepadY(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadY, sensitivity }; }
		constexpr Binding GamepadLeftBumper(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadLeftBumper, sensitivity }; }
		constexpr Binding GamepadRightBumper(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadRightBumper, sensitivity }; }
		constexpr Binding GamepadBack(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadBack, sensitivity }; }
		constexpr Binding GamepadStart(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadStart, sensitivity }; }
		constexpr Binding GamepadGuide(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadGuide, sensitivity }; }
		constexpr Binding GamepadLeftThumb(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadLeftThumb, sensitivity }; }
		constexpr Binding GamepadRightThumb(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadRightThumb, sensitivity }; }
		constexpr Binding GamepadDPadUp(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadDPadUp, sensitivity }; }
		constexpr Binding GamepadDPadRight(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadDPadRight, sensitivity }; }
		constexpr Binding GamepadDPadDown(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadDPadDown, sensitivity }; }
		constexpr Binding GamepadDPadLeft(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, ButtonIndices::GamepadDPadLeft, sensitivity }; }
		constexpr Binding GamepadCross(std::uint32_t id = 0, float sensitivity = 1.0f) { return GamepadA(id, sensitivity); }
		constexpr Binding GamepadCircle(std::uint32_t id = 0, float sensitivity = 1.0f) { return GamepadB(id, sensitivity); }
		constexpr Binding GamepadSquare(std::uint32_t id = 0, float sensitivity = 1.0f) { return GamepadX(id, sensitivity); }
		constexpr Binding GamepadTriangle(std::uint32_t id = 0, float sensitivity = 1.0f) { return GamepadY(id, sensitivity); }
	} // namespace Buttons
	...
	namespace Axes
	{
		...
		constexpr Binding GamepadLeftX(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, AxisIndices::GamepadLeftX, sensitivity }; }
		constexpr Binding GamepadLeftY(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, AxisIndices::GamepadLeftY, sensitivity }; }
		constexpr Binding GamepadRightX(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, AxisIndices::GamepadRightX, sensitivity }; }
		constexpr Binding GamepadRightY(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, AxisIndices::GamepadRightY, sensitivity }; }
		constexpr Binding GamepadLeftTrigger(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, AxisIndices::GamepadLeftTrigger, sensitivity }; }
		constexpr Binding GamepadRightTrigger(std::uint32_t id = 0, float sensitivity = 1.0f) { return Binding { EInputLocation::Gamepad, id, AxisIndices::GamepadRightTrigger, sensitivity }; }
	} // namespace Axes
} // namespace Input